...
- A date picker that pre-selects today into an empty field, or that marks today with the same clay fill it uses for the selection. Today is a label on the cell (今); the selection is a fill. Collapse the two and the user can no longer tell "I chose this" from "the form guessed".
